...
@pytest.mark.parametrize("m", [4, 8, 16, 32, 64, 128, 192, 256, 384, 512, 4096, 8192])
...
def benchmark(fn, *, warmup=3, iters=20):
    """Return mean latency in milliseconds over ``iters`` runs (eager)."""
...
def benchmark_cudagraph(fn, *, warmup=3, iters=20):
    """Mean latency (ms) measured by capturing ``fn`` in a CUDA graph and timing
    replays. Removes host-side launch/dispatch overhead so kernels are compared
    purely on GPU time. Falls back to eager timing if ``fn`` isn't capturable
    (e.g. it does a host sync like ``.item()`` / ``.cpu()`` / ``.tolist()``)."""
    # Warm up on a side stream first (compiles/autotunes; recommended pre-capture
    # so any allocations land in the graph's private pool).
    ...
    try:
        ...
        return benchmark(fn, warmup=warmup, iters=iters)
...
def test_kernelbench_inputs_match_flashinfer():
    ...
    from flashinfer.gemm import group_gemm_fp8_nt_groupwise
    import pytorch.fp8_group_gemm as mod
