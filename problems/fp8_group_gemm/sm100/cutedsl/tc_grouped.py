...
def _compiled_persist(key, mk_args, st, use_2cta):
    c = _COMPILE_CACHE.get(key)
    if c is None:
...
def grouped_fp8_gemm(
    ...
    if M % G == 0:
        ...
        def _bargs():
            return (
                from_dlpack(a_b, assumed_align=16).mark_layout_dynamic(1),
                from_dlpack(b_b, assumed_align=16).mark_layout_dynamic(1),
        ...
        if N % 128 == 0 and K % 128 == 0:
            ...
            _compiled_persist(
                ("persist_batched", m, N, K, G, use_2cta, mn), _bargs, st, use_2cta
            )(*_bargs(), st)
        else:
    ...
    for g in range(G):
        ...
        def _args(s=s, e=e, g=g, Mg=Mg):
            ...
            return (
                from_dlpack(a[s:e].reshape(Mg, K, 1), assumed_align=16).mark_layout_dynamic(1),
                from_dlpack(b[g].reshape(N, K, 1), assumed_align=16).mark_layout_dynamic(1),
