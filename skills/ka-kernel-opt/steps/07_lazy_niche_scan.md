...
Motivated by a controlled, 3x-replicated finding: agent-managed per-round
niching (tracking multiple workloads every round, checking a per-workload
champion, watching for divergence) cost more in per-round fix quality than
it recovered through protection (see `insights/DIRECTION 3 PROPOSAL -
PER-WORKLOAD NICHING.md`). This step never touches the agent's reasoning.
It runs the *exact* vanilla
...
python "${CLAUDE_SKILL_DIR}/tools/select_workloads.py" \
  --problem $RUN_DIR/problem.py --n ${NUM_SCAN_WORKLOADS:-3}
```
...
**If more than 2 distinct champions exist**, this tool's MVP doesn't handle
it — fall back to hand-authoring the dispatcher yourself (reason about the
3+ champions and their routing axes jointly) instead of forcing this
mechanical path.
