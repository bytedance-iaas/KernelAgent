#!/usr/bin/env python3
"""
Auto-select representative workloads for per-workload niching (direction 3).

Standalone: stdlib only (imports the problem file only for its WORKLOADS
list, no torch/GPU needed). See insights/DIRECTION 3 PROPOSAL - PER-WORKLOAD
NICHING.md -- every niching test so far hand-picked representative workloads
knowing roughly where the interesting regime split was; this replaces that
with a rule that runs against the problem's own WORKLOADS list (the unified
problem.md / SOL-ExecBench-style contract -- see docs/PROBLEM_MD_FORMAT.md).
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Auto-select representative workloads for niching")
