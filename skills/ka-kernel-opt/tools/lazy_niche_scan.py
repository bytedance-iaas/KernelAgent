...
def cmd_dispatch(args) -> int:
    ...
    if split is None:
        print("error: could not find a numeric axis to split on -- cannot "
              ...
              "(needs an agent-authored routing rule instead, see "
              "steps/06_dispatch_synthesis.md)", file=sys.stderr)
        return 1
    ...
    if worst_regret > SAFE_REGRET_THRESHOLD:
        print(f"  UNSAFE: worst-case regret {worst_regret:.2f}x (across "
              ...
              f"threshold, or an agent-authored one "
              f"(steps/06_dispatch_synthesis.md) -- flag for follow-up "
    ...
    if not axis_ok:
        print(f"  UNSAFE: the chosen routing axis {axis!r} does not match "
              ...
              f"but this problem's routing axis needs an agent-authored "
              f"dispatcher that knows how to extract it "
              f"(steps/06_dispatch_synthesis.md) -- flag for follow-up "
              f"rather than ship a dispatcher that silently routes on the "
              f"wrong runtime value.")
        return 1
...
workloads, or fall back to steps/06_dispatch_synthesis.md's agent-authored
routing (which can look past a single axis) if that's not acceptable.
"""
import os
import sys
